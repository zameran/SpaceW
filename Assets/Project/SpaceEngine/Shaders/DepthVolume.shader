	...
	SubShader 
	{
	    ...
	    Pass 
	    {
			...
			v2f vert (appdata v) 
			{
				...
				//v.vertex.xy z= (projectOnNearPlane * extrude > 0.0) ? LinePlaneIntersection(v.vertex.xyz, -toLight,float3(0, 0, 1), float3(0, 0, 600)) : (v.vertex.xyz = v.vertex.xyz - toLight * (extrude * 1000000));
